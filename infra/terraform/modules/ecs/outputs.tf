...
output "https_listener_arn" { value = one(aws_lb_listener.https[*].arn) }
...
output "panel_service_names" { value = { for k, s in aws_ecs_service.panel : k => s.name } }
output "panel_hosts" { value = local.panels }
