...
/// Kyçja e stafit të merchant-it. I njëjti mekanizëm si te aplikacionet;
/// serveri e lejon panelin vetëm për llogaritë që janë staf i një merchant-i (§21, §52).
...
export async function POST(req: NextRequest) {
  ...
  if (action === 'verify') {
    const res = await call('auth/otp/verify', {
      method: 'POST',
      body: JSON.stringify({ phone, code, device_name: 'Paneli i partnerit' }),
    });
