...
export default function DriversPage() {
  ...
  return (
    <>
      <PageHeader
        title="Shoferët në pritje"
        right={
          <Button onClick={() => setCreating(true)}>Regjistro shofer</Button>
        }
      />

      {creating && (
        <NewDriver onClose={() => setCreating(false)} onCreated={refresh} />
      )}
...
function DriverReview({
  ...
  async function decideDocument(id: string, action: "approve" | "reject") {
    if (action === "reject" && reason.trim().length === 0) {
      setFailure(
        "Refuzimi kërkon një arsye që shoferi ta dijë çfarë të ndreqë.",
      );
      return;
    }
    setBusy(true);
  ...
  return (
    <Drawer title={`Shoferi ${shortId(driver.user_id)}`} onClose={onClose}>
      <Row
        label="Automjeti"
        value={`${driver.vehicle_color} ${driver.vehicle_make} ${driver.vehicle_model}`}
      />
      ...
      {driver.suspended_reason && (
        <Row label="Arsyeja e pezullimit" value={driver.suspended_reason} />
      )}
        ...
        <>
          ...
          {data.documents.map((doc) => {
            ...
            return (
              ...
              >
                ...
                />
                {doc.expires_on && (
                  <Row label="Skadon" value={doc.expires_on} />
                )}
                {doc.rejection_reason && (
                  <Row label="Arsyeja" value={doc.rejection_reason} />
                )}
                <div
                  style={{
                    display: "flex",
                ...
                >
                  ...
                  {doc.status === "pending" && (
                    <>
                      <Button
                        busy={busy}
                        onClick={() => decideDocument(doc.id, "approve")}
                      >
                        Aprovo
                      </Button>
                      <Button
          ...
          {!data.eligible && (
            <p style={{ color: "var(--muted)", fontSize: 12, margin: 0 }}>
              Miratimi hapet kur të gjitha dokumentet e detyrueshme të jenë
              aprovuar.
            </p>
          )}
        </>
